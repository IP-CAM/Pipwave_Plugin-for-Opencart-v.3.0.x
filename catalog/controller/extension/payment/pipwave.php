...
class ControllerExtensionPaymentPipwave extends Controller {
    ...
    public function __construct($registry) {
        ...
        if ($this->config->get('payment_pipwave_test_mode') == '1') {
    ...
    public function index() {
        ...
        $data = array(
            ...
            'timestamp' => time(),
            'api_key' => $this->config->get('payment_pipwave_api_key'),
        ...
        if ($this->cart->hasShipping()) {
            $data['shipping_amount'] = $this->currency->format($this->tax->calculate($this->session->data['shipping_method']['cost'], $this->session->data['shipping_method']['tax_class_id'], 1), $order_info['currency_code'], $order_info['currency_value'], false);
        }
        ...
        if (isset($this->session->data['guest'])) {
            ...
            $processing_fee_ref = $this->config->get('payment_pipwave_processing_fee_ref');
            $data['buyer_info'] = array(
        ...
        $data['api_override'] = array(
            ...
            'notification_url' => $this->url->link('extension/payment/pipwave/callback', '', true),
        );

        $signatureParam = array(
            'api_key' => $this->config->get('payment_pipwave_api_key'),
            'api_secret' => $this->config->get('payment_pipwave_api_secret'),
            'txn_id' => $data['txn_id'],
            'amount' => $data['amount'],
        ...
        return $this->load->view('extension/payment/pipwave', $view_data);
    }
    ...
    private function _getOrderStatus($transaction_status, $api_status) {
        ...
        switch ($transaction_status) {
            ...
            case 1: // failed
                ...
                $order_status_id = $this->config->get('payment_pipwave_failed_status_id');
                break;
            case 2: // cancelled
                $note = "Payment Status: Cancelled$with_warning_msg";
                $order_status_id = $this->config->get('payment_pipwave_canceled_status_id');
                break;
            case 10: // complete
                $note = "Payment Status: Completed$with_warning_msg";
                $order_status_id = $this->config->get('payment_pipwave_complete_status_id');
                break;
            case 20: // refunded
                $note = "Payment Status: Refunded$with_warning_msg";
                $order_status_id = $this->config->get('payment_pipwave_refund_status_id');
                break;
            case 25: // partial refunded
                $note = "Payment Status: Partial Refunded$with_warning_msg";
                $order_status_id = $this->config->get('payment_pipwave_partial_refund_status_id');
                break;
    ...
    private function _sendRequest($data) {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('x-api-key:' . $this->config->get('payment_pipwave_api_key')));
